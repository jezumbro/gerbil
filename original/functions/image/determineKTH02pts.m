function [ p, distance ] = determineKTH02pts( pts, pxtol, greenTF)
...
% | p1          p8          p4 |
% ------------------------------
if greenTF % if green design
    g = 0.5*83.465; % mm = 3.286 inches
else
    g = 0.5*76.200; % mm = 3.000 inches
end
...
while ~stat && n<=sum(la)
    ...
    p(3,:) = pts(ind(3),:);
    theta = t1(ind(3))-pi/4;
    loc = find(2*g-pxtol <= d1 & d1 <= 2*g+pxtol);
...
la = find(g-pxtol < d11 & d11 <g+pxtol);
