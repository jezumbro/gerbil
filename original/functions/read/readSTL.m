function varargout = readSTL(file)
...
while numRead < numFaces
    ...
    if isempty(V)
        ...
        V = vertcat(V,v(~ismember(v,V,'row'),:));
    end
    % Figure out where to fit these new vertices, and the face, in the
    % larger F and V collections.
    fInd  = numRead + 1;
    vInd1 = find(ismember(V,v(1,:),'row'),1,'first');
    vInd2 = find(ismember(V,v(2,:),'row'),1,'first');
    vInd3 = find(ismember(V,v(3,:),'row'),1,'first');
    
    F(fInd,:)        = [vInd1,vInd2,vInd3];
    N(fInd,:)        = n;
